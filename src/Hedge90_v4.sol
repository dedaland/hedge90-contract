...
contract TokenSale is ReentrancyGuard {
    ...
    uint256 public buyerDiscountPercent = 1; // 1% discount for the buyer
    uint256 public referralPercent = 2; // 2% referral reward

    bool public normalBuyLocked = false;
    bool public hedge90Locked = false;
    ...
    modifier whenNormalBuyNotLocked() {
        require(!normalBuyLocked, "Normal buy is currently locked");
        _;
    }

    modifier whenHedge90NotLocked() {
        require(!hedge90Locked, "Hedge90 is currently locked");
        _;
    }
    ...
    function setTokenPriceManager(address _newTokenPriceManager) external onlyOwner {
        require(_newTokenPriceManager != address(0), "TokenPriceManager address cannot be zero address");
        tokenPriceManager = TokenPriceManager(_newTokenPriceManager);
    }

    function normalBuy(uint256 USDTAmount, address inviter) external nonReentrant whenNormalBuyNotLocked {
    ...
    function cancelHedge90(uint256 _index) external nonReentrant whenHedge90NotLocked {
    ...
    function setNormalBuyLock(bool _locked) external onlyOwner {
        normalBuyLocked = _locked;
    }

    function setHedge90Lock(bool _locked) external onlyOwner {
        hedge90Locked = _locked;
    }
    ...
    function withdrawAllFunds() external onlyOwner nonReentrant {
        ...
        if (tokenBalance > 0) {
            token.safeTransfer(withdrawalAddress, tokenBalance);
        }
}
}